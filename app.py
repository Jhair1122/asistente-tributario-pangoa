...
def generar_grafico_red():
    try:
        if modelo is None or vectorizer is None or label_encoder is None:
            ...
            ax.text(0.5, 0.5, "⚠️ Modelo no disponible", ha="center", va="center", fontsize=13, color="#888")
            ax.axis("off")
            return fig
        ...
        for i, (nombre, n, color) in enumerate(capas):
            ...
            # Etiqueta arriba
            ax.text(x_positions[i], y0 + altura + 0.06, nombre, ha="center", va="bottom", fontsize=8.5, fontweight="bold", color="#2d2d2d")
            # Número de neuronas
            ax.text(x_positions[i], y0 + altura/2, f"{n:,}", ha="center", va="center", fontsize=9, fontweight="bold", color="white")
        ...
        ax.set_ylim(0, 1.3)
        ax.axis("off")
        ax.set_title("Arquitectura del Modelo NLP — Red Neuronal MLP", fontsize=11, fontweight="bold", color="#1a3a24", pad=10)
        plt.tight_layout()
        return fig
